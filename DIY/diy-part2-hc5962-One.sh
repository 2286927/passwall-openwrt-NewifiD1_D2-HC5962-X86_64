...
#=================================================
# Modify default IP
sed -i 's/192.168.1.1/172.16.5.1/g' package/base-files/files/bin/config_generate
# DNS Cache FixUP
...
sed -i 's/ImmortalWrt/HiWiFi4E.one/g' package/base-files/files/bin/config_generate
...
sed -i '/DISTRIB_DESCRIPTION/d' package/base-files/files/etc/openwrt_release
echo "DISTRIB_DESCRIPTION='ImmortalWrt '" >> package/base-files/files/etc/openwrt_release
sed -i "s/ImmortalWrt /\洲\·\C\y \build $(TZ=UTC-8 date "+%Y.%m.%d") @ ImmortalWrt /g" package/base-files/files/etc/openwrt_release
# 修改连接数
